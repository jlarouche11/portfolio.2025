...
export default function (eleventyConfig) {
	...
	eleventyConfig.htmlTransformer.addUrlTransform(
		...
		function makeUrlRelative(urlInMarkup) {
			...
			if (!urlInMarkup || !urlInMarkup.startsWith('/') || urlInMarkup.startsWith('/.11ty/') || urlInMarkup.startsWith('//')) {
				return urlInMarkup;
			}
			...
			// Somehow, i think path.relative changes slashes to backslashes on Windows.
			// So, I had to add a replaceAll() to change them all to normal slashes.
			let relativePath = path.relative(fromDir, urlInMarkup).replaceAll('\\', '/');
